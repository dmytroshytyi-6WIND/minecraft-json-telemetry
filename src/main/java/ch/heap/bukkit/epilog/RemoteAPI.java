...
public class RemoteAPI {
	...
	private void sendTokenRequest() {
		...
		Request request = new Request("token", event.toJSON(), new RequestDelegate() {
			@Override
			public void response(boolean success, JSONObject answer) {
				String token = "niu78go7g8y7g"; // answer.optString("serverToken", null);
				String id = "pmpoimion898n9om"; // answer.optString("epilogServerID", null);
				if (token != null) {
					serverToken = token;
	...
	private void handleRequestResponse(JSONObject answer) {
		LogEvent my_event = new LogEvent("Worlds", System.currentTimeMillis(), null);
		my_event.event = new PlayerMoveEvent(null, null, null);
		long time = System.currentTimeMillis();
		my_event.time = time;
		my_event.ignore = true; // don't send back to logging server
		this.plugin.postEvent(my_event);
	...
	private void sendRequest(Request request) {
		...
		if (success) {
			...
			this.plugin.getLogger().warning(status);
	...
	public String get(String url, Map<String, String> reqProp) {
		...
		try {
			...
			if (reqProp != null)
				for (Entry<String, String> entry : reqProp.entrySet()) {
