...
public class Updater {
	...
	public void sendVersionList(CommandSender sender) {
		...
		for (String key : keys)
			all.addAll(versions.get(key).keySet());
	...
	private void checkBukkitUpdates() {
		...
		for (int i = 0; i < files.length(); i++) {
			...
			if (!file.optString("fileName").equals(fileName))
				continue;
			if (!file.optString("releaseType").equals("release"))
				continue;
			...
			// System.out.println(file.opt("name") + " " + file.opt("releaseType") + " " +
			// version);
		}
	...
	public Map<String, String> getUpdateVersions() {
		...
		if (updateFolder.isDirectory() == false)
			return versions;
		for (File file : updateFolder.listFiles()) {
			...
			if (this.getPluginNames().contains(name) == false)
				continue;
			if (!file.getName().equals(name + ".jar"))
				continue;
	...
	private boolean installPlugin(String name) {
		...
		try {
			...
			copyFile(tmp, target);
			// boolean success = tmp.renameTo(target); // doesn't work on centos; probably
			// tmp file still in use
