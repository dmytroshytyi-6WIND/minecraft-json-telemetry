...
public class PlayerNotifications {
	...
	public PlayerNotifications(Epilog epilog) {
		this.epilog = epilog;
		try {
			epilog.addEventObserver(this, this.getClass().getMethod("eventHandler", LogEvent.class),
	...
	public void updateReady() {
		inform("updateReady",
				"New plugin versions are ready to install. Type '/reload' or restart your minecraft server.", true);
	}
	...
	public void inform(String key, String message, boolean repeat) {
		...
		if (repeat == false && message.equals(messages.get(key)))
			return;
